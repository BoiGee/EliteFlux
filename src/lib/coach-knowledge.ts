...
export const CHANGELOG: ChangeDoc[] = [
  {
    ...
    detail:
      "A held coin can score High Conviction on EliteFlux's own recommendation read while Exit Intelligence, independently, flags rising distribution risk on the same coin — two genuinely true signals that previously weren't reconciled at all. The trim fired at the standard 35% regardless of what the rest of the read said. Now, when that conflict happens, the trim is lightened to a 15% hedge instead — the risk signal still gets acted on, just less aggressively, rather than either being ignored or overreacted to. This only applies to trims. A full exit (the platform's most urgent protective signal) is never softened this way, on purpose.",
  },
  {
    date: "2026-09-22",
